...
test("Blank Footer", () => {
    const result = render(React.createElement(Footer, null));
    expect(result.container).toBeValid();
});
test("Complete Footer", () => {
    const result = render(React.createElement(Footer, { companyName: "Test", companyNameLink: "http://localhost", contact: contactProps, socialMedia: socialProps }));
    expect(result.container).toBeValid();
